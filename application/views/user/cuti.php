...
<body class="lexend-deca-font bg-gray-100">
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-6 md:ml-64 mb-20 md:mb-0">
            <h2 class="text-3xl font-bold mb-8 text-gray-800 border-b pb-2">Form Pengajuan Cuti</h2>

            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
                <form action="<?= base_url('user/aksi_cuti'); ?>" method="post" id="cutiForm" class="space-y-6">
                    <div>
                        <label for="alasan-cuti" class="block text-gray-700 font-medium mb-1">Alasan Cuti</label>
                        <input type="text" id="alasan-cuti" name="alasan_cuti"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label for="awal-cuti" class="block text-gray-700 font-medium mb-1">Awal Cuti</label>
                            <input type="date" id="awal-cuti" name="awal_cuti"
                                class="border border-gray-300 w-full rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            <p id="errorAwal" class="text-red-500 text-sm mt-1 hidden"></p>
                        </div>

                        <div>
                            <label for="akhir-cuti" class="block text-gray-700 font-medium mb-1">Akhir Cuti</label>
                            <input type="date" id="akhir-cuti" name="akhir_cuti"
                                class="border border-gray-300 w-full rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
    ...
    <nav
        ...
        <a href="<?php echo base_url('user/cuti'); ?>"
            class="group flex flex-col items-center text-gray-600 hover:text-blue-600">
            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full">
                <i class="fas fa-briefcase text-lg text-blue-600 group-hover:text-blue-700"></i>
            </span>
    ...
    const awalInput = document.getElementById('awal-cuti');
    const akhirInput = document.getElementById('akhir-cuti');
    ...
    function validate() {
        ...
        if (awalDate && awalDate < today) {
            errorAwal.textContent = "Tanggal awal cuti tidak boleh sebelum hari ini.";
            errorAwal.classList.remove('hidden');
            valid = false;
        }

        if (awalDate && akhirDate && awalDate > akhirDate) {
            errorAwal.textContent = "Tanggal awal cuti tidak boleh lebih dari akhir cuti.";
            errorAwal.classList.remove('hidden');
            valid = false;
        }

        if (awalDate && akhirDate && akhirDate < awalDate) {
            errorAkhir.textContent = "Tanggal akhir cuti tidak boleh kurang dari awal cuti.";
